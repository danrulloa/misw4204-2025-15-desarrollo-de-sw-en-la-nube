...
async def list_public_videos(
    ...
    """Lista videos públicos disponibles para votación (temporalmente deshabilitado)"""
    raise HTTPException(
        status_code=status.HTTP_501_NOT_IMPLEMENTED,
...
# TODO: Descomentar cuando se actualicen los tests unitarios
# @router.get(
#     "/videos/{video_id}",
...
async def get_public_video(video_id: str):
    """Obtiene el detalle de un video público por su ID (temporalmente deshabilitado)"""
    raise HTTPException(
        status_code=status.HTTP_501_NOT_IMPLEMENTED,
        detail="Endpoint no implementado aún"
    )
...
async def vote_video(video_id: str):
    """Registra un voto para un video (temporalmente deshabilitado)"""
    raise HTTPException(
        status_code=status.HTTP_501_NOT_IMPLEMENTED,
        detail="Endpoint no implementado aún"
    )
...
async def get_rankings(
    ...
    """Obtiene el ranking de jugadores por votos totales (temporalmente deshabilitado)"""
    raise HTTPException(
        status_code=status.HTTP_501_NOT_IMPLEMENTED,
        detail="Endpoint no implementado aún"
    )
